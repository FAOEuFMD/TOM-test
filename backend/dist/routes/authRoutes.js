...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.execute("SELECT password, access_level FROM users WHERE email = ?", [email]);
        logger.debug("Database query returned %d rows", rows.length);
        if (rows.length > 0) {
            const user = rows[0];
            logger.debug("Raw stored value from DB: %s", user.password);
            ...
            if (isPasswordValid) {
                ...
                logger.warn("Failed login attempt for user %s: Invalid password.", email);
